...
public class FileComparator implements Comparator<ObjetoPersistenteVO> {
    private String str1, str2;
    private int pos1, pos2, len1, len2;

    @Override
    public int compare(ObjetoPersistenteVO s1, ObjetoPersistenteVO s2) {
        
        if (s1 instanceof AnexoArcVo && s2 instanceof AnexoArcVo) {
            str1 = ((AnexoArcVo)s1).getLink();
            str2 = ((AnexoArcVo)s2).getLink();
        } else {
            str1 = ((AnexoDocumentoVo)s1).getLink();
            str2 = ((AnexoDocumentoVo)s2).getLink();
        }
