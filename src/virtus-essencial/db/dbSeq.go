...
func createSeq() {
	...
	db.Exec("IF NOT EXISTS (SELECT 1 FROM sys.objects WHERE object_id = OBJECT_ID(N'comentarios_anotacoes_id_seq') AND type in (N'SO')) BEGIN CREATE SEQUENCE comentarios_anotacoes_id_seq AS BIGINT START WITH 1 INCREMENT BY 1 END")
	db.Exec("IF NOT EXISTS (SELECT 1 FROM sys.objects WHERE object_id = OBJECT_ID(N'comentarios_chamados_id_seq') AND type in (N'SO')) BEGIN CREATE SEQUENCE comentarios_chamados_id_seq AS BIGINT START WITH 1 INCREMENT BY 1 END")
	...
	db.Exec("IF NOT EXISTS (SELECT 1 FROM sys.objects WHERE object_id = OBJECT_ID(N'entidades_ciclos_id_seq') AND type in (N'SO')) BEGIN CREATE SEQUENCE entidades_ciclos_id_seq AS BIGINT START WITH 1 INCREMENT BY 1 END")
