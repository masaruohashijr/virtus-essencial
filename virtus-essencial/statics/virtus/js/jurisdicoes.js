...
class Jurisdicao {
	constructor(order, id, escritorioId, entidadeId, entidadeNome, entidadeSigla, iniciaEm, terminaEm, autorId, autorNome, criadoEm, idVersaoOrigem, statusId, cStatus) {
		this.order = order;
		this.id = id;
...
function addJurisdicaoRow(tableID) {
	...
	let newText = document.createTextNode(jurisdicao.entidadeSigla);
	newCell.appendChild(newText);
	newCell.style="vertical-align:middle;text-align:left";
	newCell = newRow.insertCell(1);
	newText = document.createTextNode(jurisdicao.entidadeNome);
...
function updateJurisdicaoRow(tableID, order){
	...
	for(y=0;y<linhas.length;y++){
		if(linhas[y].childNodes[0]){
			let inputOrder = linhas[y].childNodes[1].childNodes[0];
...
function deleteJurisdicao() {
	...
	var order = jurisdicao_tobe_deleted.parentNode.parentNode.childNodes[1].childNodes[0].value;
	var newJurisdicoes = [];
	let tbl = jurisdicao_tobe_deleted.parentNode.parentNode.parentNode;
	let linhas = tbl.childNodes;
	contadorLinha = 1;
	for(y=0;y<linhas.length;y++){
		if(linhas[y].childNodes[1]){
			let inputOrder = linhas[y].childNodes[1].childNodes[0];
			if(inputOrder && inputOrder.tagName=='INPUT'){ 
				if(inputOrder.value==order){
					if(inputOrder.value == order){
...
function editJurisdicao(e) {
	...
	var order = linha.childNodes[1].childNodes[0].value;
	var id = linha.childNodes[1].childNodes[1].value;
	var entidadeId = linha.childNodes[1].childNodes[2].value;
	var escritorioId = linha.childNodes[1].childNodes[3].value;
	var iniciaEm = formatarData(linha.childNodes[2].innerText);
	console.log(iniciaEm);
	var terminaEm = formatarData(linha.childNodes[3].innerText);
